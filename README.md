...
2. The first time, `-fix` args can't auto-fix `*_test.go` file, you need to run the command once again, this is a bug of golang/x/tools, see golang/go#54740 for more information. (ps: This problem occurs with basically all linters that use the `go/analysis` package), In this case, my advice is to run the linter command multiple times.:sweat_smile:
...
4. If package doc like above include duplicated word, `-fix` will wrong, see golang/go#54774 for more information.
```go
/*
package a
